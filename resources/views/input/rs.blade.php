...
<head>
  ...
  <title>Input RS — Sitrep NTT</title>
...
<body class="min-h-screen px-4 py-8" style="background:#EEF1F0;font-family:Inter,system-ui,sans-serif;">
  <div class="max-w-2xl mx-auto">
    ...
    <h1 class="text-2xl font-semibold text-slate-800 mt-2 mb-6">🩺 Input Data RS</h1>
    ...
    <form method="POST" action="{{ route('input.rs.store') }}" class="bg-white rounded-xl p-6 shadow-sm space-y-3">
      @csrf
      <div>
      ...
      <div>
        <label class="block text-sm text-slate-600 mb-1">Nama RS</label>
        <input type="text" name="nama_rs" required class="w-full border border-mint-200 rounded-lg px-3 py-2">
      </div>
      <div>
